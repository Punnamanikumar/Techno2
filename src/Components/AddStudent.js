...
const AddStudent = () => {
  ...
  const handleSubmit = (e) => {
    if (name ===''|| birthdate==='' || grade==='' || mentor===''|| email==='' || password==='' || stid==='') {
  ...
  return (
    <div className="inputfleids">
      <Box className="flex2" component="form" sx={{ "& > :not(style)": { m: 2, width: "40ch" } }} noValidate autoComplete="off" >
        ...
        <TextField className="in5" id="outlined-basic" value={email} onChange={(e) => setEmail(e.target.value)} label="Email" variant="outlined"/>
        <TextField className="in6" id="outlined-basic" value={password} onChange={(e) => setPassword(e.target.value)} label="Password" variant="outlined"/>
        <TextField className="in7" id="outlined-basic" value={stid} onChange={(e) => setStid(e.target.value)} label="Stuednt ID" variant="outlined"/>
    </Box>
      <button className="close"><Link to="/home" style={{ textDecoration: "none", color: "black" }}> Close </Link> </button>
